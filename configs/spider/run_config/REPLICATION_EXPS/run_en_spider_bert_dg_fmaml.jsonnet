{
    ...
    logdir: "log/en_spider_fmaml/bert_value_%d" %exp_id,
    ...
    eval_output: "ie_dir/en_spider_bert_fmaml",
    eval_beam_size: 3,
    eval_debug: false,
    eval_name: "%s_bert_run_%d_%s_%s_%d" % [self.project, exp_id, self.eval_section, self.eval_method, self.eval_beam_size],

    local _start_step = $.model_config_args.save_threshold / 1000,
    local _end_step = $.model_config_args.max_steps / 1000,
    eval_steps: [ 100 * x for x in std.range(_start_step, _end_step)],
}
