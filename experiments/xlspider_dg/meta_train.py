...
from tensor2struct.training import dgreptile
from tensor2struct.training.data_scheduler import DBReptileScheduler
from experiments.spider_dg.meta_train import MetaTrainConfig

class MetaTrainerXL(meta_train.MetaTrainer):
    def load_train_config(self):
        ...
        if self.train_config.num_batch_accumulated > 1:
            raise ValueError("First Order Methods should not use accumulation due to outer-step bug")
    ...
    def load_optimizer(self, config):
        with self.init_random:
            ...
            maml_trainer = dgreptile.MAML(
                model=self.model, 
                inner_opt=inner_optimizer, 
                inner_steps=self.train_config.inner_steps,
                first_order=self.train_config.first_order,
                do_cross_gradient_normalization=self.train_config.do_cross_gradient_normalization,
                device=self.device
            )
    ...
    def step(
        ...
        with self.model_random:
            for _i in range(self.train_config.num_batch_accumulated):
                ...
                inner_batches, outer_batch = task
                ret_dic = maml_trainer.meta_train(
                    self.model, inner_batches, outer_batch
                )
                loss = ret_dic["loss"] # This is for reporting
...
def main(args):
    ...
    trainer = MetaTrainerXL(logger, config)
